...
本リポジトリには、スマートメーターと直接通信する**親機プログラム**と、親機からESPNOWで送られてくるデータを別のM5デバイスの
画面に表示する**子機（モニター）プログラム**の、大きく2種類が含まれています。
それぞれ対象ボードも設定項目も異なるため、本READMEも「親機について」
「子機について」の章を分けて説明しています。
...
- 加速度センサーによる画面表示の自動回転（重力方向を検知して0/90/180/270度に追従。対応プログラムのみ）
- ESPNOWによる取得データのブロードキャスト配信（親機→子機）
- [Ambient](https://ambidata.io/) へのデータ送信（任意、瞬間電力値/積算電力量を別チャンネルで送信。親機のみ）
- SoftAP + ブラウザによる設定ポータル（WiFi・各種しきい値などを、設定ファイルの
  編集なしでブラウザから入力可能。設定値は内蔵Flash(NVS)に保存。親機・子機共通）
- UNIT Buzzer（M5Stack社製、Groveポート接続の外付けブザー）への対応
- BP35A1・BP35C1-J11-T01の両方に対応（親機のみ、コンパイル時に切替）

## 必要なライブラリ

- **M5Unified**（M5Stack公式、Arduino Library Managerから導入。親機・子機共通で必要）
- **Ambient ESP32 ESP8266 lib**（Ambient公式、Library Managerで"Ambient"を検索。
  **親機のみ**で使用。Ambient連携を使わない場合もincludeしているためインストールは必要です）
- ESP32 Arduino core 同梱: `WiFi.h`, `WebServer.h`, `DNSServer.h`,
  `Preferences.h`, `esp_now.h`（親機・子機共通）
...
Wi-SUNモジュール(BP35A1/BP35C1)経由でスマートメーターと直接通信し、
瞬時電力値・積算電力量を取得して画面表示、およびESPNOW/Ambientへ配信する
メインのプログラムです。
...
## 対象ボード・対象モジュールの選択（コンパイル時設定）

`wisun_Arduino.ino` の冒頭に、ボードとWi-SUNモジュールそれぞれの選択スイッチが
あります。使用する組み合わせの行だけコメントを外し、他はコメントアウト
してください（ボード・モジュール、それぞれ必ず1つだけ有効にすること）。
...
1. 書き込み後、初回起動時は自動的に設定ポータル（SoftAP）が起動します。
   （2回目以降、設定をやり直したい場合は **ボタンAを2秒間長押し**）
...
端末を譲渡・廃棄する際など、認証情報を残したくない場合にご利用ください。
...
親機（`wisun_Arduino`）がESPNOWでブロードキャストする瞬時電力値・積算電力量を
受信し、別のM5デバイスの画面に表示するモニター用（子機）プログラムです。
[元となったMicroPython/UIFlow版](https://github.com/rin-ofumi/m5stickc_wisun_hat)の
`test_WiSUN_view_m5stickc.py` / `test_WiSUN_view_m5stack.py` / `test_WiSUN_view_m5core2.py`
に相当します。対象ボードの系統ごとに、それぞれ1つのプログラムへ統合しています。

| プログラム | 対象ボード |
|---|---|
| `wisun_Arduino_view_StickC` | M5StickC / M5StickC Plus / M5StickC Plus2 / M5StickS3 / M5AtomS3 / M5AtomS3R |
| `wisun_Arduino_view_M5Stack` | M5Stack (Basic/Gray) / M5Stack Core2 / M5Stack CoreS3 |

いずれも、親機と同じフォルダ構成の考え方（`.ino` + 設定ポータル用ヘッダー）で、
SoftAP + ブラウザによる設定ポータルから設定します。**WiFi SSID/パスワードは
親機と同じアクセスポイントを指定してください。**ESPNOWは同一チャンネル同士でしか
通信できないため、親機・子機がそれぞれ同じWiFi APに接続することで、通信
チャンネルを一致させています（インターネット接続自体が必須というわけではありません）。

設定ポータルには、親機と同様に警報しきい値（契約ブレーカー値・警告係数・
無応答タイムアウト）と、全設定初期化（端末譲渡・廃棄時用）を用意しています。

## wisun_Arduino_view_StickC（M5StickC系・M5AtomS3系）

```
wisun_Arduino_view_StickC/
├── wisun_Arduino_view_StickC.ino   … メインスケッチ
└── ViewConfigStickC.h              … 設定値の保存(NVS)＋Web設定ポータル
```

- ボード選択（`TARGET_BOARD_*`）・BEEP(ブザー)の扱い・画面自動回転
  （`BOARD_ROTATION_OFFSET`、IMUによる4方向自動追従）は、親機と全く同じ設計です。
- 瞬時電力値のみを大きく表示します（積算電力量の表示はありません。元の
  MicroPython版`test_WiSUN_view_m5stickc.py`と同じ範囲です）。
- ボタン操作: ボタンA シングルクリック=BEEP ON/OFF切替、ボタンB シングル
  クリック=画面自動回転のON/OFF切替、**ボタンAを2秒間長押し=設定ポータル起動**
  （親機・M5Stack系子機と統一した操作方法です）。

## wisun_Arduino_view_M5Stack（M5Stack系）

```
wisun_Arduino_view_M5Stack/
├── wisun_Arduino_view_M5Stack.ino  … メインスケッチ
└── ViewConfigM5Stack.h             … 設定値の保存(NVS)＋Web設定ポータル
```

- 画面解像度(320x240)・ボタン操作（A/B/C。Core2/CoreS3はタッチをM5Unifiedが
  仮想ボタンとして自動エミュレート）が対象ボード間で共通のため、ボード選択の
  切替は不要です。
- 瞬時電力値の大画面表示と、30分毎積算電力量の棒グラフ表示（前日+当日の
  96コマ分）を、ボタンBで切り替えられます。元のMicroPython版
  (`test_WiSUN_view_m5stack.py` / `test_WiSUN_view_m5core2.py`)のロジックを
  踏襲しています。
- ブザーはM5Stack(Basic/Gray)・Core2・CoreS3いずれも内蔵スピーカーを搭載して
  いるため、外付けブザーは不要です。警告音の周波数は親機と共通の定数
  （`BUZZER_TONE_FREQ_HZ`、既定4000Hz）を使用します。
- 画面は横長固定です（元のMicroPython版にも回転機能はなく、棒グラフ等の
  レイアウトが横長前提のため踏襲しています）。
- ボタン操作: ボタンA シングルクリック=バックライトのミュート切替、ボタンB
  シングルクリック=表示モード切替（瞬時電力値⇔棒グラフ）、ボタンC シングル
  クリック=BEEP ON/OFF切替、**ボタンAを2秒間長押し=設定ポータル起動**。
  Core2/CoreS3はボタンAがタッチ式の仮想ボタンとなり、電源ON時点の押下状態を
  正しく取得できないため、親機・`wisun_Arduino_view_StickC`と同様に「起動後の
  長押し」の方式にしています（M5Stack Basic/Grayでも同じ操作方法です）。

## ESPNOWメッセージ形式

`NPD=xxx`（瞬時電力値）、`TPD=xxx/日時`（積算電力量）という文字列形式で
親機からブロードキャストされます。`wisun_Arduino_view_StickC`は`NPD=`のみ、
`wisun_Arduino_view_M5Stack`は両方を処理します。自作の子機を作る場合も、
`esp_now_register_recv_cb()`で受信データの先頭が`"NPD="`か`"TPD="`かを
見て処理を分岐してください。

## 実機での確認ポイント（子機）

- 親機と同じWiFi APに接続できているか（ESPNOWのチャンネルが親機と一致
  している必要があります）。設定ポータルでWiFi SSID/パスワードを親機と
  同じものにしているかご確認ください。
- 画面回転が想定と異なる場合（`wisun_Arduino_view_StickC`のみ）は、
  `BOARD_ROTATION_OFFSET`または`updateAutoRotation()`内の判定部分を
  親機と同様に調整してください。
- 内蔵ブザー/UNIT Buzzerの音の聞こえ方は個体差があるため、聞き取りにくい
  場合は`BUZZER_TONE_FREQ_HZ`（既定4000Hz）を変更して調整してください。
- M5Unifiedのボタン関数名（`wasClicked()`等）がお使いのバージョンと
  一致しているかは、親機と同様にご確認ください。
...
- 画面描画は座標を細かく指定する方式ではなく、M5GFXの`setTextDatum`/
  `drawString`による中央揃え表示を基本にしています（一部、子機の棒グラフ
  表示など座標指定が必要な箇所を除く）。表示位置を細かく調整したい場合は
  各プログラムの描画関数（`drawPower()`/`drawPowerValue()`等）を変更してください。
...
### Rev 0.0.3 【2026/7/22】
- 子機（モニター）プログラム `wisun_Arduino_view_StickC`
  （M5StickC / Plus / Plus2 / StickS3 / AtomS3 / AtomS3R向け）、
  `wisun_Arduino_view_M5Stack`（M5Stack Basic/Gray / Core2 / CoreS3向け）
  を追加。いずれも親機と同じ設計思想（Web設定ポータル、フォント・表示倍率
  設定、全設定初期化機能）を踏襲しています。
- 設定ポータルの起動方法を、「起動時にボタンAを押しながら電源ON」から
  「起動後にボタンAを2秒間長押し」に統一（親機・両子機共通）。
  Core2/CoreS3のタッチ式仮想ボタンは、電源ON直後の押下状態を正しく
  取得できないため、この方式に変更しています。
- 設定ポータル(SoftAP)へ切り替える際、ESP-NOWおよびWiFi STA接続の
  後始末（停止処理）を、実際にそれらが初期化・起動されている場合のみ
  行うよう修正。未設定（工場出荷状態）の端末でESP-NOWやWiFiが一度も
  起動されていない段階から後始末処理を実行してしまい、ウォッチドッグ
  タイマーによる意図しない再起動を繰り返す不具合があったため対応
  しました（`espNowInitialized`/`wifiStarted`フラグで初期化・起動済み
  かどうかを追跡）。
- `wisun_Arduino_view_M5Stack`の電力値表示について、フォント・表示倍率の
  Web設定への対応、レイアウト（数値と単位"W"の位置関係、表示モード
  切替時に受信済みの値が即時表示されない問題）を修正。
- バックライト輝度の初期値を255（最大）に変更（環境によっては低い
  初期値だと画面が見えづらいことがあったための対応）。
