...
void startSetupPortal() {
  ...
  if (wifiStarted) {
    // 既にSTA接続等でWiFiドライバが動いていた場合のみ、AP切替前に
    // きちんと後始末する。一度もWiFiを起動していない(真っさらな)状態で
    // disconnect()を呼ぶと、機種によってはドライバが不安定になり
    // フリーズ(ウォッチドッグによる意図しない再起動)することがあるため、
    // 必要な場合のみ呼ぶようにしている（未設定の初回起動時に必須の対策）。
...
// （handleSetupLongPress()参照。子機プログラムと統一）。
